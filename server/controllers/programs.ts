...
export async function programsRoute(body: any, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.url?.lastIndexOf('/') != undefined && req.url?.lastIndexOf('/') > 0) {
  ...
  getAllPrograms(body, req, res);

}

export async function getAllPrograms(body: any, req: IncomingMessage, res: ServerResponse): Promise<void> {
  console.log('all programs', body);
  const groupNumbers = await getIntervals(body.id);
